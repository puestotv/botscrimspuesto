...
module.exports.run = async (bot, message, args) => {
    ...
    let startMessage = new Discord.RichEmbed()
        ...
        .setDescription("Merci d'écrire les 3 derniers codes de votre identifiant serveur !")
        .setColor("#8600b3")
        .setFooter("PuestoTV");
    ...
    let timeEmbed = new Discord.RichEmbed()
        .setTitle("Prochaine game en approche..")
    ...
    collector.on('collect', m => {
        ...
        if (game.data.length === 0 && m.content.length === 3){
            ...
            if (game.userPresent(m.author.username)){
                game.deletedUserEntry(m.author.username);
                ...
                if (game.idPresent(m.content.toUpperCase())){
                    game.addUser(m.content.toUpperCase(), message.author.user);
                }else {
                    game.addID(m.content.toUpperCase(), m.author.username);
                }
            }
